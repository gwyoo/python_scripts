#!/bin/bash
#
# snmp install script for CentOS-7
# version 0.9
#
# scripted by gwyoo
# 2016. 8. 11
...
############## cat executable file's PATH CHECK is VERY IMPORTANT !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
##############
echo "extend     system        /usr/bin/cat  /etc/snmp/dmidecode_t_1.log                    " >> /etc/snmp/snmpd.conf
echo "extend     processor     /usr/bin/cat  /etc/snmp/dmidecode_t_4.log                    " >> /etc/snmp/snmpd.conf
...
echo
echo -n "IF \"Active: active (running)\" is not, then press Q + [Enter]. Otherwise [Enter] "
...
echo
echo -n "IF \"active\" is not, then press Q + [Enter]. Otherwise [Enter] "
...
firewall-cmd --list-all | grep "161/udp" > /dev/null
if [[ $? = 1 ]]; then
...
echo
echo -n "IF \"ports: 161/dup\" is not, then press Q + [Enter]. Otherwise [Enter] "
